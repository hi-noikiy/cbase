...
class Ftp
{
	...
	public function __call($name, $args)
	{
        ...
        {
			CLog::write("Not connected to FTP server. Call connect() or ssl_connect() first.", CLog::NOTICE, "ftp");
        ...
        else 
        {
            ...
            if($func === 'ftp_login' && !$res)
            {
                CLog::write("500 Access Defined to FTP server. Call login() . user: {$args[1]} pass:{$args[2]}", CLog::NOTICE, "ftp");
                throw new FtpException("Not connected to FTP server. Call login(). user:{$args[1]} pass:{$args[2]}");
            }
		...
		if (!$silent && $this->errorMsg !== NULL) 
        {
            ...
            CLog::write($this->errorMsg, CLog::NOTICE, "ftp");
...
class FtpException extends CException
{
