...
class Date
{
       public static $week_zh = array( 0 => '日', 1 => '一', 2 => '二', 3=> '三', 4 => '四', 5 => '五', 6 => '六');
	/**
	 * 人性化地显示时间，新浪微博的实现
     ...
     * @param $time unix时间戳 
     **/
    public static function getCurrentMonth($time = '')
    {
    ...
    /**
     * 获取给定一个时间的 +-月份的开始时间和结束时间
     * @param $month +1/-1 整数
     * @param $time unix时间戳 
     **/
    public static function getMonth($month, $time)
    {
        $op = $month > 0 ? '+' : '-';
        $month = $month > 0 ? $month : -$month;
        $start_time = strtotime(date('Y-m-01',strtotime("{$op}{$month} month", $time)));
        $end_time   = strtotime("+1 month -1 day", $start_time);
        return array($start_time, $end_time); 
    }
    ...
    /**
     * 获取和当前时间 - $days的开始和结束时间
     * @param $time unix时间戳 
     * @param $day  +1/-1 整数
     **/
    public static function getLatelyDays($day, $time)
    {
        $day  = intval($day);
        $op = $day > 0 ? '+' : '-';
        $end_time = strtotime(date('Y-m-d', $time));
        $start_time   = strtotime("{$day} day", $end_time);
        return array($start_time, $end_time); 
    }
}